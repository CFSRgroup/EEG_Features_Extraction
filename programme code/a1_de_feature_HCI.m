...
for sub = 1:24
for zq=1:20 %20次试验
eval(['[data,numChan,labels,txt,fs,gain,prefiltering,ChanDim] = eeg_read_bdf(''D:\EEGRecognition\dataset\dataset_HCI\Part_' num2str(sub) '_S_Trial' num2str(zq) '_emotion.bdf'',''all'',''n'');'])
%%
%下采样256-128Hz
...
eeg_de(1,i)=de_entropy(eeg_segment_single);
end
eeg_temp=eeg_de;%汇总eeg时域特征160
%%
%汇总所有特征364
feature((zq-1)*3+z1,:)=eeg_temp;
end
end
eval(['save D:\EEGRecognition\Project_2103\features\de\HCI\p' num2str(zq) ' feature'])
end
