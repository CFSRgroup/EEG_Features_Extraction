...
for session=1:3
eval(['load 'D:\EEGRecognition\dataset\dataset_SEED\' num2str(subject) '_' num2str(session) ''])
...
% end
for i1=1:32;%此处有循环
deentropy(ns,i1)=de_entropy(data_seg(:,i1));
end
end

de((1+(trial-1)*3)+(session-1)*45:(trial*3)+(session-1)*45,:)=deentropy;%eeg频域特征
end
end
feature=double(de);
eval(['save D:\EEGRecognition\Project_2103\features\de\SEED\p' num2str(subject) ' feature'])
end